      ...
      >
        <div className="px-5 lg:px-8 xl:px-10 max-lg:py-4">
          <a className="block text-2xl lg:text-2xl font-normal xl:text-3xl xl:mr-8">
            Travelling <span className="text-yellow-500 font-bold">NEMO</span>
          </a>
        </div>
      </div>
      <nav
        ...
        } fixed top-0 left-0 right-0 bottom-0 lg:ml-auto lg:static lg:flex`}
      >
        <div
          onClick={handleClick}
          ...
          {navigationData.map((item: NavigationItem) => (
            <Link
              ...
              className={`block relative font-bold text-center text-4xl lg:font-normal lg:text-xl xl:text-2xl
               tracking-wide px-20 py-2 transition-colors hover:text-yellow-500 lg:pt-0 lg:mx-4 lg:p-0 ${
                 false ? "text-yellow-500 underline underline-offset-4" : ""
...
const SmallDevicesMenu: React.FC<SmallDevicesMenuProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`fixed border border-yellow-500 w-10 transition-all ${
          openNavigation ? "rotate-45 mt-4" : ""
        }`}
      />
      <div
        className={`fixed border border-yellow-500 w-10 mt-2 transition-all ${
          openNavigation ? "hidden" : ""
        }`}
      />
      <div
        className={`fixed border border-yellow-500 w-10 mt-4 transition-all ${
